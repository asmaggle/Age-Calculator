...
function calculate() {
  ...
  //check the input value is blank or not
  if (inputData !== "") {
    ...
    if (day === currentDate && month === currentMonth) {
      ...
      output.innerHTML = `Wohoo! Today Is your birthday your are <span class="year">${birthday}</span> Years Old. `;
    } else {
      ...
      output.innerHTML = `        You are <span class="year">${year}</span> Year <span class="month">${month}</span> Month <span class="day">${day}</span> Days old!`;
    }
  }
